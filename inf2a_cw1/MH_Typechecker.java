...
class MH_Typechecker {
	...
	static MH_TYPE computeType (MH_EXP exp, TYPE_ENV env) throws TypeError, UnknownVariable {
		if (exp.isNUM()) {
			...
			MH_TYPE argType = computeType(exp.second(), env);
			if (argType.equals(funcType.left())) {
				return funcType.right();
			} else {
				throw new TypeError("Argument type does not match the function type.");
			}
		} else if (exp.isINFIX()) {
			if (computeType(exp.first(), env).equals(IntegerType)) {
				if (computeType(exp.second(), env).equals(IntegerType)) {
					...
					throw new TypeError("Right operand is not an Integer.");
				}
			} else {
				throw new TypeError("Left operand is not an Integer.");
			}
		} else if (exp.isIF()) {
			if (computeType(exp.first(), env).equals(BoolType)) {
				MH_TYPE type1 = computeType(exp.second(), env);
				MH_TYPE type2 = computeType(exp.third(), env);
				if (type1.equals(type2)) {
					return type1;
				} else {
					throw new TypeError("Then statement has a different type than else statement.");
				}
			} else {
				throw new TypeError ("If condition is not a boolean expression.");
			}
		} else {
