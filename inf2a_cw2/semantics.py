...
def sem(tr):
    ...
    if (tr.node == 'P'): # correctamundo
        return tr[0][0]
    elif tr.node == 'N' or tr.node == 'A' or tr.node == 'I': # correctamundo
    ...
    # S Expansions
        ...
        return sem(tr[1])
    elif rule == 'S -> WHICH Nom QP QM': # correctamundo
        return '(\\x.(' + sem(tr[1]) + '(x) & ' + sem(tr[2]) + '(x)))'

    # QP Expansions
    elif rule == 'QP -> VP': # correctamundo
        return sem(tr[0])
    elif rule == 'QP -> DO NP T': # correctamundo
        return '(\\x. exists y.(' + sem(tr[1]) + '(y) & ' + sem(tr[2]) + '(x)(y)))'

    # VP Expansions
    elif rule == 'VP -> T NP': # correctamundo
        return '(\\x. exists y.(' + sem(tr[1]) + '(y) & ' + sem(tr[0]) + '(y)(x)))'
    elif rule == 'VP -> BE NP' or rule == 'VP -> BE A': # correctamundo
        return '(\\x.' + sem(tr[1]) + '(x))'
    elif rule == 'VP -> VP AND VP': # correctamundo
        return '(\\x.(' + sem(tr[0]) + '(x) & ' + sem(tr[2]) + '(x)))'
    elif rule == 'VP -> I': # correctamundo
        return '(\\x' + sem(tr[0]) + '(x))'

    # NP Expansions
    elif rule == 'NP -> P': # correctamundo
        return '(\\x.(x=' + sem(tr[0]) + '))'
    elif rule == 'NP -> AR Nom': # correctamundo
        return sem(tr[1])
    elif rule == 'NP -> Nom': # correctamundo
        return sem(tr[0])

    # Nom Expansions
    elif rule == 'Nom -> AN': # correctamundo
        return sem(tr[0])
    elif rule == 'Nom -> AN Rel': #correctamundo
        return '(\\x.(' + sem(tr[0]) + '(x) & ' + sem(tr[1]) + '(x)))'

    # AN Expansions
    elif (rule == 'AN -> A AN'): #correctamundo
        return '(\\x.(' + sem(tr[0]) + '(x) & ' + sem(tr[1]) + '(x)))'
    elif (rule == 'AN -> N'): #correctamundo
        return sem(tr[0])

    # Rel Expansions
    elif rule == 'Rel -> WHO VP': # correctamundo
        return '(\\x.' + sem(tr[1]) + '(x))'
    elif rule == 'Rel -> NP T': # correctamundo
